<header style="
  ...
  <div style="
    ...
    if ($connected) {
      $firstname = $this->request->getAttribute('identity')->firstname;
      $name = $this->request->getAttribute('identity')->name;
      $admin = $this->request->getAttribute('identity')->admin;
      ?>
    <div style="position:absolute; left: 3%; top: 70%">
      <a style="color: black; font-weight: 900;cursor: pointer; padding: 10 20; background-color: white; border-radius: 10px;" href="/messages-history/history"> Historique</a>
      <a> <?= $firstname ?> </a>
      <a> <?= $name ?> </a>
    </div>
      <?php if ($admin == 0) {
...
<div class="screens">
  <div class="screen screen2">
    <div class="parallax-wrap">
      ...
      <div class="moon" id="moon7">
        <div></div>
      </div>
      <div class="moon" id="moon8">
        <div></div>
      </div>
  ...
  <div class="screen screen1">
    <div class="parallax-wrap">
      ...
      <div class="moon" id="moon1">
        <div></div>
      </div>
      <div class="moon" id="moon2">
        <div></div>
      </div>
      <div class="moon" id="moon3">
        <div></div>
      </div>
  ...
  <div class="screen screen3">
    <div class="parallax-wrap">
      ...
      <div class="moon" id="moon4">
        <div></div>
      </div>
      <div class="moon" id="moon5">
        <div></div>
      </div>
      <div class="moon" id="moon6">
        <div></div>
      </div>
      <div class="moon" id="moon9">
        <div></div>
      </div>
...
<div id="popupIa" class="popup open">
  ...
  <div class="popup-content">
    ...
    if ($connected) { ?>
      <div>
        <input type="text" id="message" name="message" class="input" placeholder="Décrivez votre rêve...." autocomplete="off">
      </div>
      <div>
        <button class="buttonClick" id="envoyer-button">Envoyer</button>
      </div>
    <?php } else { ?>
      <div style="width: 100%; text-align: center;  left: 48%">
        <h3 style="color:white; z-index:99;">Connectez vous pour pouvoir utiliser l'IA Onirix</h3>
      </div>
        ...
        $('#envoyer-button').click(function() {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              var response = $('<div class="response"><p></p></div>').appendTo('#chat-messages');
              var letters = data.split("");
              var i = 0;
              var printInterval = setInterval(function() {
                if (i < letters.length) {
                  response.find('p').append(letters[i]);
                  i++;
                } else {
